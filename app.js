...
function readMotionVars() {
  ...
  IMAGE_BASE_SCALE = parseFloat(styles.getPropertyValue('--image-base-scale')) || 1.22;
  IMAGE_SCALE_RANGE = parseFloat(styles.getPropertyValue('--image-scale-range')) || 0.05;
}
readMotionVars();
...
function tick() {
  ...
  rects.forEach(({ top, height }, i) => {
    ...
    copies[i].style.setProperty('--copy-shift', `${parallax}px`);
    copies[i].style.opacity = 1;
  });
